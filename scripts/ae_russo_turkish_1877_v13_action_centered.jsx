/*
  ...
  - camera staging is derived from the active route/node, so action stays centered;
  - tiny movements use MACRO.
*/

(function () {
    app.beginUndoGroup("Create Russo-Turkish War 1877 V13 Action Centered");
    ...
    var FPS = 30;
    var DURATION = 92;
    var comp = app.project.items.addComp("Russo_Turkish_War_1877_V13_Action_Centered_92s", W, H, 1, DURATION, FPS);
    ...
    function cameraRoute(rig, t, coords, scale) {
        ...
        cameraFocus(rig, t, f[0], f[1], scale, 960, 555);
    }

    function cameraNode(rig, t, lon, lat, scale) {
        cameraFocus(rig, t, lon, lat, scale, 960, 555);
    }
    ...
    cameraRoute(rig, 12, ROUTE_KISHINEV_UNGHENY, PLAN.MED);
    cameraRoute(rig, 19, ROUTE_KISHINEV_UNGHENY, PLAN.MED);
    cameraRoute(rig, 24, ROUTE_ZIMNITSA_SVISHTOV, PLAN.MED);
    cameraRoute(rig, 29, ROUTE_ZIMNITSA_SVISHTOV, PLAN.MACRO);
    cameraRoute(rig, 37, ROUTE_ZIMNITSA_SVISHTOV, PLAN.MACRO);
    cameraRoute(rig, 42, ROUTE_SVISHTOV_NIKOPOL, PLAN.MED);
    cameraRoute(rig, 50, ROUTE_SVISHTOV_NIKOPOL, PLAN.MED);
    cameraRoute(rig, 55, ROUTE_SVISHTOV_TARNOVO_SHIPKA, PLAN.MED);
    cameraRoute(rig, 66, ROUTE_SVISHTOV_TARNOVO_SHIPKA, PLAN.MED);
    ...
    route("r_svishtov_tarnovo_shipka", ROUTE_SVISHTOV_TARNOVO_SHIPKA, COL.red, 55.5, 62.8, 67.0);
    ...
    text("ui_footer_title", "Русско-турецкая война 1877. V13: камера над действием.", [58, 970], 23, COL.gold, ParagraphJustification.LEFT_JUSTIFY, false, false);
    text("ui_footer_note", "Правило теста: если есть маршрут или узел, фокус камеры берётся из координат этого действия.", [58, 1002], 19, COL.muted, ParagraphJustification.LEFT_JUSTIFY, false, false);

...
(function () {
    app.beginUndoGroup("Create Russo-Turkish War 1877 V15 Cinematic Commanders");
    ...
    var FPS = 30;
    var DURATION = 92;
    var comp = app.project.items.addComp("Russo_Turkish_War_1877_V15_Cinematic_Commanders_92s", W, H, 1, DURATION, FPS);
    ...
    function addMapPng(name, fileObj, lon, lat, scale, tIn, tOut, dx, dy) {
        ...
        var layer = comp.layers.add(footage);
        layer.name = name;
        layer.property("Transform").property("Position").setValue([p[0] + (dx || 0), p[1] + (dy || 0)]);
        inverseScaleBase(layer, scale);
    ...
    function mapMedallion(name, fileObj, lon, lat, label, role, tIn, tOut, dx, dy, side) {
        ...
        var x = p[0] + dx;
        var y = p[1] + dy;
        var med = addMapPng(name + "_portrait_medallion", fileObj, lon, lat, 28, tIn, tOut, dx, dy);
        if (!med) return null;
        pulseRing(name, lon, lat, side === "hostile" ? COL.red : COL.blue, tIn + 0.2, tOut, 32);
        var labelLayer = shadowText(name + "_card_label", label, [x, y + 75], 18, COL.cream, tIn + 0.15, tOut, ParagraphJustification.CENTER_JUSTIFY, true, true);
        var roleLayer = shadowText(name + "_card_role", role, [x, y + 96], 13, side === "hostile" ? [1.0, 0.72, 0.62] : [0.72, 0.86, 1.0], tIn + 0.22, tOut, ParagraphJustification.CENTER_JUSTIFY, true, true);
        return med;
    }
    ...
    mapMedallion("cmd_nikolai", new File(portraitDir.fsName + "/nikolai_nikolaevich_medallion.png"), 28.32, 47.10, "Николай Николаевич", "русская армия", 9.2, 20.0, 6, -92, "friendly");
    tacticalIcon("ico_kishinev_column", new File(tacticalDir.fsName + "/cavalry_blue.png"), 28.32, 47.10, 10.2, 19.4, -28, 42, 15);
    ...
    mapMedallion("cmd_carol", new File(portraitDir.fsName + "/carol_i_medallion.png"), 25.362, 43.644, "Кароль I", "румынский союзник", 26.0, 38.0, -92, -74, "friendly");
    tacticalIcon("ico_crossing_flag", new File(tacticalDir.fsName + "/objective_flag_blue.png"), 25.358, 43.638, 27.4, 37.5, 56, -14, 13);
    ...
    tacticalIcon("ico_nikopol_fort", new File(tacticalDir.fsName + "/fort_gold.png"), 24.900, 43.700, 40.0, 51.0, 48, -54, 14);
    tacticalIcon("ico_nikopol_cannon", new File(tacticalDir.fsName + "/cannon_blue.png"), 25.16, 43.67, 42.0, 50.4, -6, 42, 13);
    ...
    mapMedallion("cmd_gurko", new File(portraitDir.fsName + "/iosif_gurko_medallion.png"), 25.50, 43.02, "Иосиф Гурко", "передовой отряд", 53.0, 67.0, 90, -86, "friendly");
    tacticalIcon("ico_gurko_cavalry", new File(tacticalDir.fsName + "/cavalry_blue.png"), 25.47, 42.91, 54.0, 66.5, -64, -38, 15);
    tacticalIcon("ico_shipka_hill", new File(tacticalDir.fsName + "/hill_fort_gold.png"), 25.320, 42.750, 56.0, 67.0, 52, 18, 15);
    ...
    mapMedallion("cmd_osman_move", new File(portraitDir.fsName + "/osman_pasha_medallion.png"), 23.48, 43.78, "Осман-паша", "османская армия", 68.8, 79.0, 18, -104, "hostile");
    tacticalIcon("ico_osman_cannon", new File(tacticalDir.fsName + "/cannon_red.png"), 23.48, 43.78, 70.0, 78.4, -52, 42, 14);
    ...
    mapMedallion("cmd_osman_siege", new File(portraitDir.fsName + "/osman_pasha_medallion.png"), 24.617, 43.417, "Осман-паша", "оборона Плевны", 80.5, 91.5, -118, -92, "hostile");
    mapMedallion("cmd_skobelev", new File(portraitDir.fsName + "/mikhail_skobelev_medallion.png"), 24.617, 43.417, "Михаил Скобелев", "штурмы Плевны", 82.0, 91.5, 124, -92, "friendly");
    tacticalIcon("ico_plevna_fort", new File(tacticalDir.fsName + "/fort_gold.png"), 24.617, 43.417, 80.5, 91.5, 0, 58, 15);
    tacticalIcon("ico_plevna_swords", new File(tacticalDir.fsName + "/crossed_swords_gold.png"), 24.617, 43.417, 82.0, 91.0, 0, -58, 15);
    ...
    text("ui_footer_title", "Русско-турецкая война 1877. V14: action viewport + правильный geo-parent.", [58, 970], 23, COL.gold, ParagraphJustification.LEFT_JUSTIFY, false, false);
    text("ui_footer_note", "Тест: активный маршрут должен быть в рабочем центре кадра и занимать заметную часть карты.", [58, 1002], 19, COL.muted, ParagraphJustification.LEFT_JUSTIFY, false, false);

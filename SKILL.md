...
- `scripts/ae_russo_turkish_1877_v19_midline_symbols_zones.jsx`: current working AE JSX template with corrected PNG projection origin, reference-style political map staging, action-viewport centering, commander/avatar route heads, midline tactical pictograms, captured/contested-zone overlays, stable local symbol anchors, and geo-parenting via `setParentWithJump`.
...
- `scripts/render_v19_preview_frames.py`: renders fast Pillow preview frames for projection-lock, action-centering, route-head avatars, midline tactical symbols, and operation-zone readability before opening AE.
...
- Run `render_v19_preview_frames.py` or an equivalent visual QA pass before handing over a larger scripted animation.
